...
final class BooksControllerRevokeBookTask
  ...
  public Unit onHeldReady(final OPDSAvailabilityHeldReady a)
    ...
    a.getRevoke().mapPartial_(
        new PartialProcedureType<URI, IOException>() {
          @Override
          public void call(final URI revoke_uri)
              throws IOException {
  ...
  private void revokeUsingURI(
    ...
    final HTTPAuthType auth = AccountAuthenticatedHTTP.createAuthenticatedHTTP(this.getAccountCredentials());
  ...
  public Unit onHeld(final OPDSAvailabilityHeld a)
    ...
    a.getRevoke().mapPartial_(
        new PartialProcedureType<URI, IOException>() {
          ...
          public void call(final URI revoke_uri)
              throws IOException {
            revokeUsingURI(revoke_uri, RevokeType.HOLD);
          }
        });
    return Unit.unit();
  }
  ...
  private void revokeLoanedWithDRM(
    ...
    OptionType<AccountAuthenticationCredentials> creds_opt =
        this.accounts_database.accountGetCredentials();

    if (creds_opt.isNone()) {

      /*
       * No credentials are available. This almost certainly indicates a bug in the program.
       */

      final OptionType<Throwable> no_exception = Option.none();
      this.revokeFailed(no_exception, "No credentials are available!");
      return;
    }
    ...
    final OptionType<AccountAuthenticationAdobePostActivationCredentials> adobe_creds_post_opt =
        creds.adobePostActivationCredentials();

    if (adobe_creds_post_opt.isNone()) {
      ...
      final OptionType<Throwable> no_exception = Option.none();
      this.revokeFailed(no_exception, "No Adobe post-activation credentials are available!");
      return;
    }

    final AccountAuthenticationAdobePostActivationCredentials adobe_creds_post =
        ((Some<AccountAuthenticationAdobePostActivationCredentials>) adobe_creds_post_opt).get();
      ...
      adobe.execute(
          new AdobeAdeptProcedureType() {
            ...
            public void executeWith(final AdobeAdeptConnectorType c) {
              c.loanReturn(listener, loan.getID(), adobe_creds_post.userID());
            }
          });
  ...
  public void returnBookWithoutDRM(
