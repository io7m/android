...
final class BooksControllerBorrowTask implements Runnable {
  ...
  private void downloadFailed(
    ...
    exception.map_(
        new ProcedureType<Throwable>() {
          @Override
          public void call(final Throwable x) {
            LOG.error("[{}]: download failed: ", sid, x);
          }
        });
  ...
  private void saveEPUBAndRights(
    ...
    final BookDatabaseEntryType e = this.books_database.databaseOpenEntryForWriting(this.book_id);
    ...
    final BookStatusType downloaded_status = BookStatus.fromSnapshot(this.book_id, downloaded_snap);
  ...
  private void runFulfillACSM(
      final File file)
      throws IOException,
      AdobeAdeptACSMException,
      BookUnsupportedTypeException {
  ...
  private void runFulfillACSMWithConnector(
    ...
    /*
     * XXX: Nothing, at this point, appears to have ensured that the device is activated.
     *      Presumably this is an assumption that has crept into the code undocumented by the
     *      previous maintainer, so I've added some precondition checks here to ensure that when
     *      this assumption is eventually broken, it doesn't get broken quietly.
     */

    final OptionType<AccountAuthenticationCredentials> creds_opt =
        this.accounts_database.accountGetCredentials();

    Assertions.checkPrecondition(
        creds_opt.isSome(), "Credentials must be provided!");

    final AccountAuthenticationCredentials creds =
        ((Some<AccountAuthenticationCredentials>) creds_opt).get();

    Assertions.checkPrecondition(
        creds.hasActivatedAdobeDevice(), "Device must be activated!");

    final AccountAuthenticationAdobePostActivationCredentials creds_post =
        ((Some<AccountAuthenticationAdobePostActivationCredentials>) creds.adobePostActivationCredentials()).get();
    ...
    adobe.execute(
         ...
         */
            ...
            downloads.put(book_id, new DownloadType() {
              @Override
              public void cancel() {
            ...
            c.fulfillACSM(new AdobeFulfillmentListener(), acsm, creds_post.userID());
          }
        });
  }
          ...
          if (this.adobe_drm.isSome() && this.needs_auth) {
            ...
            if (credentials_opt.isSome()) {
              ...
              if (credentials.hasActivatedAdobeDevice()) {
                ...
                /*
                 * XXX: Why is device activation happening here? Why is the device activated
                 *      and then no borrow/download actually performed?
                 */
  ...
  private void runAcquisitionBorrow()
    ...
    final HTTPAuthType auth =
        AccountAuthenticatedHTTP.createAuthenticatedHTTP(this.getAccountCredentials());
  ...
  private AccountAuthenticationCredentials getAccountCredentials() {
    final OptionType<AccountAuthenticationCredentials> credentials_opt =
        this.accounts_database.accountGetCredentials();
    ...
    return ((Some<AccountAuthenticationCredentials>) credentials_opt).get();
  }
  ...
  private DownloadType runAcquisitionFulfillDoDownload(
    ...
    if (this.needs_auth) {
      auth = Option.some(AccountAuthenticatedHTTP.createAuthenticatedHTTP(
...
//          }
  ...
  private final class AdobeFulfillmentListener implements AdobeAdeptFulfillmentListenerType {
